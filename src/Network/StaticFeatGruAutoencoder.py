...
class StaticFeatGruAutoencoder(nn.Module):
    """
        Parameters：
    ...
    def forward(self, to_x, xTimestampSizes):
        ...
        x = paddedX[:,paddedX.shape[1] - self.reserveLengthForDecode:paddedX.shape[1] ,:]
        x = self.lstmEncoderForward(x)
        paddedX = x.repeat(1,to_x.shape[1],1)
