...
class OffsetBiGruAutoencoder(nn.Module):
    ...
    def __init__(self, feature_size, hidden_size=1, output_size=1, num_layers=1, ahead=1):
        ...
        self.flstmEncoder = nn.GRU(feature_size, hidden_size, num_layers,batch_first =True, bidirectional=True) # utilize the LSTM model in torch.nn 
        self.rlstmEncoder = nn.GRU(feature_size, hidden_size, num_layers,batch_first =True, bidirectional=True)
