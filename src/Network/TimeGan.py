...
class TimeGan(nn.Module):
    ...
    def __init__(self, feature_size, hidden_size=1, output_size=1, num_layers=1):
        ...
        self.staticFeatureModel = nn.GRU(feature_size, hidden_size, num_layers,batch_first =True, bidirectional=False)
        self.tempFeatureModel = nn.GRU(feature_size, hidden_size, num_layers,batch_first =True, bidirectional=True) # utilize the LSTM model in torch.nn 
        self.lstmDecoder = nn.GRU(2*hidden_size, hidden_size, num_layers, batch_first=True, bidirectional=True) 
        
        self.generator = nn.GRU(2*hidden_size, hidden_size, num_layers, batch_first=True, bidirectional=True)
    ...
    def forward(self, to_x, xTimestampSizes):
        ...
        staticFeature = self.staticFeatureModel(x)
        tempFeature = self.tempFeatureModel(x)
