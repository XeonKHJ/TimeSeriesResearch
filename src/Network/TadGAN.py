...
class TadGAN(nn.Module):
    ...
    def __init__(self, feature_size, hidden_size=1, output_size=1, num_layers=1):
        ...
        self.generator = nn.Sequential(
            nn.GRU(feature_size, hidden_size, num_layers,batch_first =True, bidirectional=True)
        )

        self.discriminator = nn.Sequential(
            nn.GRU(hidden_size, hidden_size, num_layers, batch_first=True, bidirectional=True) 
        )
