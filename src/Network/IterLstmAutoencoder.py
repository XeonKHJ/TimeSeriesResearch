...
class IterLstmAutoencoder(nn.Module):
    """
        Parameters：
    ...
    def __init__(self, feature_size, hidden_size=1, output_size=1, num_layers=1):
        ...
        self.gruEncoder = nn.LSTM(feature_size, hidden_size, num_layers,batch_first =True) # utilize the LSTM model in torch.nn 
        self.encodeFc = nn.Linear(hidden_size, hidden_size)
        self.gruDecoder = nn.LSTM(hidden_size, hidden_size, num_layers, batch_first=True) 
    ...
    def forward(self, to_x, xTimestampSizes, outputLength=None):
        ...
        x, (hiddenOutput, hiddenCellStatus) = self.gruEncoder(x)  # _x is input, size (seq_len, batch, input_size)
        ...
        encoded, (encodedH,hiddenStatus) = self.gruDecoder(encoded, (hiddenOutput, hiddenCellStatus))
        decoderOutput = encoded
        if outputLength == None:
            outputLength = to_x.shape[1]
        for idx in range(1, outputLength):
            decoderOutputUnit, (encodedH,hiddenStatus) = self.gruDecoder(decoderOutput[:, idx-1, :].reshape(paddedX.shape[0], 1, -1), (encodedH, hiddenStatus))
