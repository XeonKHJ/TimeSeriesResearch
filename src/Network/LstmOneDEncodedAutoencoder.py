...
class LstmOneDEncodedAutoencoder(nn.Module):
    """
        Parameters：
    ...
    def __init__(self, feature_size, hidden_size=1, output_size=1, num_layers=1):
        ...
        self.lstmEncoder = nn.LSTM(feature_size, self.encodedFeatureSize, num_layers,batch_first =True) # utilize the LSTM model in torch.nn 
        self.lstmEncoderForward = nn.Linear(self.encodedFeatureSize, self.encodedFeatureSize)
        self.relu = nn.ReLU()
        self.lstmDecoder = nn.LSTM(self.encodedFeatureSize, hidden_size, num_layers, batch_first=True) 
    ...
    def forward(self, to_x, xTimestampSizes):
        ...
        x, (hiddenOutputX, _) = self.lstmEncoder(x)  # _x is input, size (seq_len, batch, input_size)
