...
class BiWindowTrainer(ITrainer):
    def __init__(self, forwardModel, backwardModel, taskName, logger, learningRate=1e-3, showTrainningInfo=True):
        self.fowardModel = forwardModel
        self.backwardModel = backwardModel
        self.lossFunc = torch.nn.MSELoss()
        self.optimizer = torch.optim.Adam(self.fowardModel.parameters(), lr=learningRate)
    ...
    def train(self, trainSet, lengths, labelSet):
        self.fowardModel.train()
        startTime = time.perf_counter()
        preSet = trainSet[:,0:int(trainSet.shape[1]/2), :]
        latterSet = trainSet[:,int(trainSet.shape[1]/2):trainSet.shape[1], :]

        fowardOutput = self.fowardModel(preSet, lengths / 2)
        backwardOutput = self.backwardModel(preSet, lengths/2)
        loss = self.lossFunc(fowardOutput, latterSet)
        loss = self.lossFunc(backwardOutput, preSet)
    ...
    def evalResult(self, validDataset, validsetLengths, labels):
        self.fowardModel.eval()
        reconstructData = self.reconstruct(self.fowardModel, validDataset, validsetLengths)
    ...
    def recordResult(self, dataset, lengths, storeNames):
        ...
        for validIdx in range(len(lengths)):
            validOutput = self.reconstruct(self.fowardModel, dataset, lengths)
    ...
    def save(self):
        ...
        torch.save(self.fowardModel.state_dict(), path.join(globalConfig.getModelPath(), filename))

    def load(self):
        filename = self.modelName + ".pt"
        self.fowardModel.load_state_dict(torch.load(path.join(globalConfig.getModelPath(), filename))) 
